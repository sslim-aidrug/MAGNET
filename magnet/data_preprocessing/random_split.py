...
def random_split_df(
    ...
    stratify_reduce: str = "any"    
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Randomly split DataFrame row indices into (train, val, test).
    - sizes: (train, val, test) proportions must sum to 1
    - when stratify=True:
        * classification: build a label vector from target_list, then stratify
        * regression: bin target_list (length 1 recommended) via qcut, then stratify
    - returns: three numpy int64 arrays (row indices)
    """

    assert len(sizes) == 3, "sizes must be a 3-tuple (train, val, test)."
    s = float(sizes[0]) + float(sizes[1]) + float(sizes[2])
    if not np.isclose(s, 1.0):
        raise ValueError(f"sizes do not sum to 1: {sizes} (sum={s})")

    n = len(df)
    if n == 0:
        return np.array([], dtype=np.int64), np.array([], dtype=np.int64), np.array([], dtype=np.int64)

    all_idx = np.asarray(df.index.values, dtype=np.int64)

    strat_labels_all = None
    if stratify:
        if target_list is None or len(target_list) == 0:
            raise ValueError("target_list must be provided when stratify=True.")

        if task_type == "classification":
            Y = df[target_list].values
            if Y.ndim == 1 or Y.shape[1] == 1:
                strat_labels_all = Y.reshape(-1)
            else:
                if stratify_reduce == "any":
                    strat_labels_all = (Y == 1).any(axis=1).astype(int)
                elif stratify_reduce == "all":
                    strat_labels_all = (Y == 1).all(axis=1).astype(int)
                elif stratify_reduce == "sum":
                    strat_labels_all = (Y == 1).sum(axis=1).astype(int)
                else:
                    raise ValueError("stratify_reduce must be one of 'any'|'all'|'sum'.")

        elif task_type == "regression":
            tcol = target_list[0]
            y = df[tcol].values.astype(float)
            strat_labels_all = pd.qcut(y, q=min(regression_bins, max(1, len(np.unique(y)))),
    ...
    train_idx, temp_idx = train_test_split(
        ...
        random_state=seed,
        shuffle=True,
        stratify=(strat_labels_all if strat_labels_all is not None else None))
    ...
    val_ratio_in_temp = sizes[1] / (sizes[1] + sizes[2])

    strat_labels_temp = None
    if strat_labels_all is not None:
        pos = {ix: i for i, ix in enumerate(all_idx)}
        temp_pos = np.array([pos[ix] for ix in temp_idx], dtype=np.int64)
        strat_labels_temp = np.asarray(strat_labels_all)[temp_pos]
    ...
    val_idx, test_idx = train_test_split(
        ...
        random_state=seed,  
        shuffle=True,
        stratify=(strat_labels_temp if strat_labels_temp is not None else None))
...
def print_split_info(name, df, train, val, test, target_list=None, task_type="classification"):
    """
    Print split sizes and label distribution per dataset.
    classification -> label distribution per split
    regression     -> sample count per split
    """
    print(f"\n{name} : total {len(df)} | train {len(train)}, val {len(val)}, test {len(test)}")

    if task_type == "regression":
        print(f"  train: {len(train)} samples")
        print(f"  valid: {len(val)} samples")
        print(f"  test : {len(test)} samples")
        return

    label_cols = [c for c in target_list if c in df.columns] if target_list else []

    def _to_df(x):
        return x if isinstance(x, pd.DataFrame) else df.loc[x]

    train_df, val_df, test_df = _to_df(train), _to_df(val), _to_df(test)

    for split_name, split_df in zip(["train", "valid", "test"], [train_df, val_df, test_df]):
        if len(label_cols) == 1:
            col = label_cols[0]
            counts = split_df[col].value_counts(normalize=True) * 100
            p0 = counts.get(0, 0)
            p1 = counts.get(1, 0)
            print(f"  {split_name} 0: {p0:.2f}% | 1: {p1:.2f}%")
        elif len(label_cols) > 1:
            print(f"  {split_name} multi-label ({len(label_cols)} tasks):")
            for col in label_cols:
                counts = split_df[col].value_counts(normalize=True) * 100
                p0 = counts.get(0, 0)
                p1 = counts.get(1, 0)
                print(f"    {col} → 0: {p0:.2f}% | 1: {p1:.2f}%")
        else:
            print(f"  {split_name} : no labels")


def check_disjoint(train_idx, val_idx, test_idx, n_total, name="dataset"):
    """Verify that train, val, test indices are disjoint."""
    s_tr, s_va, s_te = set(train_idx.tolist()), set(val_idx.tolist()), set(test_idx.tolist())
    assert s_tr.isdisjoint(s_va), f"[{name}] train and val overlap!"
    assert s_tr.isdisjoint(s_te), f"[{name}] train and test overlap!"
    assert s_va.isdisjoint(s_te), f"[{name}] val and test overlap!"
    assert len(s_tr | s_va | s_te) == len(s_tr) + len(s_va) + len(s_te), f"[{name}] duplicate indices found!"
    assert len(train_idx) + len(val_idx) + len(test_idx) == n_total, f"[{name}] size sum differs from total!"
    print(f"[{name}] verified - train/val/test disjoint ({len(train_idx)}, {len(val_idx)}, {len(test_idx)})")


def save_split_indices(name, train_idx, val_idx, test_idx, seed, split_type, dir):
    """Save train/val/test indices to {dir}/{name}/{name}-{split_type}-{seed}.npz."""
    ...
    save_path = os.path.join(save_dir, f"{name}-{split_type}-{seed}.npz")
    np.savez_compressed(save_path, train=train_idx, val=val_idx, test=test_idx)

    print(f"[{name}] saved → {save_path} "
          f"(train={train_idx.size}, val={val_idx.size}, test={test_idx.size})")

def load_all_in_one_pickle(pickle_path):
    """
    Load the pickle file at the given path and return all stored variables.
    """
...
if __name__ == '__main__':
    ...
    if not current_target_list:
        ...
        train_idx, val_idx, test_idx = random_split_df(
            df=filtered_data, 
